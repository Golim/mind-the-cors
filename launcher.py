...
def main():
    ...
    parser.add_argument('-s', '--sites',
        help='Sites list (csv with ranking, or simple txt list)', required=True)

    parser.add_argument('-m', '--max',        default=MAX,
        help=f'Maximum number of sites to test concurrently (default: {MAX})')

    parser.add_argument('-a', '--arguments',  default='--reproducible',
    ...
    denylist = ['acm.com'] # Because they IP-baned us for doing research?!

    sites = []
    try:
        with open(args.sites, 'r') as f:
            if args.sites.endswith('.csv'):
                sites = [s.strip().split(',')[1] for s in f.readlines()]
            else:
